import { ReelSymbolImages, ReelSymbolType } from "./slot_symbols.js";

// scaffolding a refactored reel builder, each function will be in its own file

ReelSymbolImages;

export function _preloadImages(ReelSymbolImages: ReelSymbolType[]) {
...
export function _chooseReelSymbols(
  ReelSymbolImages: ReelSymbolType[],
  ReelSymbolAmount: number,
  TToggleRandomSwitch: boolean
) {
  ...
  const reelSymbols: ReelSymbolType[] = [];
  const reelSymbolOrderIndex: number[] = [];
  let loop = 0;

  if (TToggleRandomSwitch) {
    // randomize the order of the symbols
    for (let i = 0; i < ReelSymbolAmount; i++) {
      const symbolSelector = Math.floor(
        Math.random() * ReelSymbolImages.length
      );
...
export function _makeCanvasImage(ChoosenReelSymbols: ReelSymbolType[]) {
